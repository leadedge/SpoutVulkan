...
void HelloTriangle::render_triangle(uint32_t swapchain_index)
{
	...
	receiver.ReceiveImage(context.gpu, context.device, cmd,
			swapchain_images[swapchain_index],    // Swapchain image
			VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,      // Swapchain image layout
			context.swapchain_dimensions.format,  // Swapchain vulkan format
			context.swapchain_dimensions.width,
			context.swapchain_dimensions.height); // Swapchain image size
	//
	#else

	// Set clear color values.
	// SPOUT
	// Clear for unorm format rather than srgb
	...
	//
	sender.SendImage(context.gpu, context.device, cmd,
	                 swapchain_images[swapchain_index],
		             VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
	                 context.swapchain_dimensions.width,
                     context.swapchain_dimensions.height,
		             context.swapchain_dimensions.format); // Backbuffer format
	//
...
HelloTriangle::~HelloTriangle()
{
	...
	#ifdef BUILDRECEIVER
	receiver.ReleaseVulkanImage(context.device);
	#else
	sender.ReleaseVulkanImage(context.device);
	#endif
