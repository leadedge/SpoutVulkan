...
public:
	...
	// Use different object names for clarity
	#ifdef BUILDRECEIVER
	spoutVK receiver;
	#else
	spoutVK sender;
	...
	VulkanExample() : VulkanExampleBase()
	{
		...
		// SPOUT
		#ifdef BUILDRECEIVER
		title += " (RIGHT click to select sender)";
		#else
		title += " (Vulkan Triangle Sender ";
		title += std::to_string(width) + "x" + std::to_string(height) + ")";
		// Give the sender a name
		// The executable name is used by default
		sender.SetSenderName("Vulkan Triangle Sender");
		#endif
	...
	~VulkanExample() override
	{
		...
		if (device) {
			...
			// SPOUT
			// Free the linked Vulkan image
			#ifdef BUILDRECEIVER
			receiver.ReleaseVulkanImage(device);
			#else
			sender.ReleaseVulkanImage(device);
			#endif

		}

		// SPOUT
		// The D3D11 texture is released and 
		// DirectX11 closed in the SpoutVK destructor
		#ifdef BUILDRECEIVER
	...
	void render() override {
		...
		// so we must use the acquire function and can't just cycle through the images/imageIndex on our own
		uint32_t imageIndex;
		...
		// ReceiveImage
		//
		// o Receive a shared texture and handle from a sender
		// o If the size has changed, create a Vulkan image and link with
		//   the sender D3D11 texture memory using the share handle
		// o If not, the linked Vulkan image is updated from the share handle
		// o Copy the linked image to the receiving (swapchain) image
		// o Fit to the receiving image if the size is different
		receiver.ReceiveImage(physicalDevice, device,
			commandBuffers[currentFrame],
			swapChain.images[imageIndex],    // Swapchain image
			VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, // Swapchain image layout
			swapChain.colorFormat,           // Swapchain vulkan format
			width, height);                  // Swapchain image size
		// =============================================
		...
		// SPOUT
		// Frame rate is uncapped for these examples
		// and can be controlled here if necessary
		receiver.HoldFps(60);
		...
		// o Create a sender of format corresponding to the swapchain colour format
		// o For a size change
		//   - update the D3D11 texture and linked Vulkan image
	    //   - update the sender information
		// o Get access to the shared texture
		// o Copy the swap chain image to the linked Vulkan image
		//   to update the sender's shared texture.
		// o Copy the swap chain image to the linked Vulkan image
		// o Signal a new frame for receivers
		//
		sender.SendImage(physicalDevice, device, commandBuffer,
		     swapChain.images[imageIndex], VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
		     width, height, swapChain.colorFormat);
		// =========================================

		// Ending the render pass will add an implicit barrier transitioning
		// the frame buffer color attachment to VK_IMAGE_LAYOUT_PRESENT_SRC_KHR
		// for presenting it to the windowing system
		...
		vk::CommandBuffer currentFrameCommandBuffer = vk::CommandBuffer(commandBuffers[currentFrame]);
...
// Most of the code base is shared for the different supported operating systems,
// but stuff like message handling differs
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (vulkanExample != NULL)
	{
		#ifdef BUILDRECEIVER
		// SPOUT
		// Right mouse click to select sender
		if (uMsg == WM_RBUTTONUP) {
			// Centre on the application window
			vulkanExample->receiver.SelectSender(hWnd);
		}
		#endif
